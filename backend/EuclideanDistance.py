...
def EuclideanDistance(data:pd.DataFrame=None, target_idx:int=None, file_name='', normalized:int=None):
    '''
    data: pd.DataFrame()
    target_idx: int
    file_name: str
    ...
    for j in range(len(feature_data)):
        ...
        distances.append((j, distance, file_names[j], labels[j]))

    # 將結果轉為 DataFrame 並排序
    distance_df = pd.DataFrame(distances, columns=['Index', 'Distance', 'FileName', 'Label']).sort_values(by='Distance', ascending=True)
...
if __name__ == '__main__':
    ...
    print(EuclideanDistance(file_name='BWimage_047.jpg', normalized=1))
